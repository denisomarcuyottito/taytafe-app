...
export const RestaurantFoodInfo = ({
  ...
  const setOrder = (action: "+" | "-", { menuId, price }: Menu) => {
    const itemIndex = orderItems.findIndex((item: OrderItem) => item.menuId === menuId);
    if (itemIndex > -1) {
      const item = orderItems[itemIndex];
      item.qty = action === "+" ? item.qty + 1 : item.qty > 0 ? item.qty - 1 : item.qty;
  ...
  const getOrderQty = (menuId: number): number => {
    return orderItems.find((item: OrderItem) => item.menuId === menuId)?.qty || 0;
  };

  const getBasketItemCount = () => orderItems.reduce((a, b) => a + (b.qty || 0), 0);
        ...
        {restaurant?.menu.map((item: Menu, index: number) => (
          <View key={`menu-${index}`} style={styles.container}>
            <View style={styles.menuContainer}>
              {/* Food image */}
              <Image source={item.photo} resizeMode="cover" style={styles.menuImage} />
            ...
            <ScrollView style={{ zIndex: -1 }}>
              ...
              <View style={styles.caloriesContainer}>
                ...
                <Text style={styles.caloriesText}>{item.calories.toFixed(2)} cal</Text>
              </View>
            </ScrollView>
          </View>
        ))}
